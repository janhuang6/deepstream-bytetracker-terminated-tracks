...
// ByteTracker construction/destruction (unchanged)
...
// Main update: standard ByteTrack logic (unchanged, see original repo)
...
// MODIFIED: Probe function for terminated tracks (used in DeepStream integration)
vector<STrack> BYTETracker::probe(const vector<NvObject>& nvObjects) {
    this->frame_id++;
    vector<STrack> detections;
    vector<STrack> detections_low;
...
// MODIFIED: Returns tracks removed since last frame (for terminated tracks output)
vector<STrack> BYTETracker::getDifferentialRemovedTracks() {
    vector<STrack> tracks_to_return = sub_stracks(removed_stracks, pre_removed_stracks);
    pre_removed_stracks.clear();
    pre_removed_stracks = removed_stracks;
